# Current Handoff - 2026-09-08 DAU-first unapproved guide repair

- Goal: current unapproved fleet descending DAU; Todaypharm rank1/84 activeUsers on September7. Existing AdSense official exact state GETTING_READY23:13:18Z; account review not submitted. No approval guarantee or whole-site certification.
- Fresh source: this established E release clone clean at dd26f6045af36202bbab4771f8dc0b813b80c90f, fresh fetch HEAD=origin/main, exact Production6307845758success. Old original checkout preserved. Previous holiday article is already live; old pre-release paragraph below is historical.
- Data gate: naturally scheduled sync34164713562 at21:55Z reported pharmacy rows26086->26127/new41 and public sample verified. No inference of41newbusinesses; no sync/publish/DB job dispatched and no new article justified solely by a count delta.
- New confirmed defect: current public /guide/night-weekend advertises2km and nonexistent status tabs, versus actual nearby3/5/10km and distance/closing sorting. Seven scoped copy corrections now match actual controls, keep TOP3 under selected sort, keep phone/registered-hours caveat. FAQ/HowTo reuse visible data. No medical advice, legal/privacy policy, identity, ads, API or DB schema change.
- Files for release: src/app/guide/night-weekend/page.tsx; tests/unit/remediation.test.ts; .gitignore (exclude skill cache); this docs/HANDOFF.md. Ignored .seo-cache contains scoped content/technical evidence, never commit it.
- Fresh checks: new rendered regression RED on old3km claim then33/33GREEN; initial3require-import lint errors corrected with staticimports, lint and both tsc checks nowexit0; buildexit0/58outputs; diffcheckpass. Local production browser23:31:09Z HTTP200/390and1280/nooverflow/oneH1/canonical/FAQ3andHowTo3visibleparity, guide->nearby and radius5/10/3 clicks pass; pageErrors0. Main reviewed mobile screenshot; independent Astra no material findings. No GPS/search-result/actual-opening proof. Existing E2E suite not rerun locally for text-only change; ordinary Git CI remains next.
- Side effects: local test memory DB only, empty productionDB env for build/server. Local browser initially attempted third-party resources; bounded proof used blocking. Earlier live read had ad403 and affiliate telemetryCORS errors, not diagnosed/fixed here; no ad clicked. Browser todaypharm-fleet-astra and owned localserver session23799/port53161 still open at this checkpoint; close after live verification. No secrets copied/printed.
- Release status: ready for explicit four-file commit and ordinary main push, then observe exact-SHA Git CI/Production and canonical live page. No Vercel API/CLI mutations. Recovery: normal narrow revert of resulting source commit, never reset dirty original. Ignore local fixture .next for deployment; Git sourcebuild only.
- Dependency audit: fresh npm audit --omit=dev --audit-level=high exits0 but reports2moderate packages (qs and sanitize-html); not a zero-vulnerability result. No package/lockfile changed. Review exposure and exact safe upgrade separately from this copy-only repair; do not run force-fix automatically.
- Next: perform sourceGit release and live-after verification, then resume fleet in DAU order. Full current fleet goal remains active; other user sessions preserved.

# Previous Handoff - 2026-09-07 AdSense factual correction
