...
4. **고유성 규칙**: summary 첫 문장에 **(a) 주소의 고유 토큰(도로명+번지/건물명/층·호 등) 중 1개 이상** + **(b) 오늘 또는 대표 영업시간(예: 09:00~20:00)**를 반드시 포함
...
{
  "summary": "2-3문장. 첫 문장에 '${pharmacy.name}' + '${fullAddress}'의 고유 요소(도로명/번지/건물명/층·호 중 1개 이상) + '오늘 영업시간(${todayHours?.open && todayHours?.close ? `${formatHHMM(todayHours.open)}~${formatHHMM(todayHours.close)}` : "정보 없음"})'을 포함. 홍보/추천 문구 금지.",
  "detailed_description": "3-4문장. 주소('${fullAddress}')를 1회 명시하고, 요일별/주말/공휴일 운영의 핵심 패턴을 사실 기반으로 설명. 추론 금지(모르면 '확인 필요').",
  "bullets": [
    "영업시간(평일/토요일/일요일/공휴일) 핵심 패턴을 1문장으로(시간 포함)",
    "주소의 고유 요소(도로명/번지/건물명/층·호)를 포함한 위치 정보 1문장",
    "전화번호 또는 '정보 없음'을 포함한 이용 정보 1문장",
    "오늘 상태('${status.label}')를 포함한 방문 전 확인 포인트 1문장"
  ],
  "local_tips": [
    "방문 전 확인 포인트 1개(예: 운영시간/전화 확인 등) - '권장/추천' 금지",
    "영업시간 정보가 없거나 변동 가능성 안내 1개(사실 기반)",
    "지도 검색/길찾기 활용 팁 1개(중립적)"
  ],
  "nearby_landmarks": [
